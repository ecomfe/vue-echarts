...
- `option: object`
  ...
  - 其他情况下，Vue ECharts 会分析差异：组件身份变更/删除及匿名组件内部的属性删除会按需使用 `replaceMerge`；已按 ID 匹配的组件内部属性删除、非组件数组缩短及其他高风险变更会退回 `notMerge: true`。
...
- `autoresize: boolean | { throttle?: number, onResize?: () => void }`（默认值：`false`）
  ...
  图表渲染容器尺寸变化时是否自动调整图表大小。也可以传入一个选项对象来指定自定义的节流延迟和尺寸变化时的额外回调函数。容器从零尺寸恢复时会立即同步，以确保首个可见帧使用正确尺寸。
